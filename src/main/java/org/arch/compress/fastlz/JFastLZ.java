...
public class JFastLZ {
	...
	protected static final byte[] SIXPACK_MAGIC = { (byte) 137, '6', 'P', 'K', 13, 10, 26, 10 };

	protected static final String DEFAULT_ARCHIVE_EXTENSION = ".flz";

	/*
	 * Always check for bound when decompressing. Generally it is best to leave it defined/true.
	 */
	...
	public int fastlzCompress(byte[] input, int inOffset, int inLength, byte[] output, int outOffset, int outLength)
			throws IOException {
		return fastlzCompress(inLength < (1024 * 64) ? JFastLZLevel.One : JFastLZLevel.Two, input, inOffset, inLength,
				output, outOffset, outLength);
	}

	public int fastlzCompress(JFastLZLevel jfastLZLevel, byte[] input, int inOffset, int inLength, byte[] output,
			int outOffset, int outLength) throws IOException {
		...
		while (ip < ipLimit) {
			...
			if (!matchLabel) {
				...
				if (distance == 0
						|| (jfastLZLevel == JFastLZLevel.One ? (distance >= JFastLZ.MAX_DISTANCE)
								: (distance >= JFastLZ.MAX_FARDISTANCE))
			...
			/*
			 * literal: output[outOffset + op++] = input[inOffset + anchor++]; ip = anchor; copy++; if(copy ==
			 * MAX_COPY){ copy = 0; output[outOffset + op++] = MAX_COPY-1; }
			 */
		}
