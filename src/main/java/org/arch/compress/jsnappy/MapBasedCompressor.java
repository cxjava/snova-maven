...
class MapBasedCompressor {
	...
	static SnappyBuffer compress(byte[] in, int offset, int length, SnappyBuffer out, boolean useFirstHit) {
	...
	private static Hit search(byte[] source, int index, int length, IntListHashMap map, boolean useFirstHit) {
		...
		if (useFirstHit) {
			...
			while (ii.next()) {
				...
				if (offset >= 4) {
					int l = 0;
					for (int o = index - offset, io = index; io < length && source[o] == source[io] && o < index
							&& l < 64; o++, io++) {
						l++;
					}
					if (res == null) {
