...
/**
 * This class provide utility methods for compressing data blocks using the Snappy algorithm.
 * 
...
public class SnappyCompressor {
	 ...
	 *            data to be compressed
	 * @param out
	 *            Buffer for compressed data block
	 ...
	 * @param out
	 *            output buffer or null (new buffer will be allocated)
	 * @return reference to <code>out</code>
	 */
	...
	/**
	 * <p>
	 * Compress the data contained in <code>in</code> from <code>offset</code> and <code>length</code> bytes. If an
	 * output buffer is provided, the buffer is reused for the compressed data. If the buffer is too small, its capacity
	 * is expanded to fit the result. If a <code>null</code> argument is passed, a new buffer is allocated.
	 * </p>
	 * 
	 * <p>
	...
	public static SnappyBuffer compress(byte[] in, int offset, int length, SnappyBuffer out, int effort) {
		...
		if (effort < 1 || effort > 100) {
			throw new IllegalArgumentException(
					"Compression effort must be an integer from 0 (fastest, less compression) to 100 (slowest, highest compression)");
		}
