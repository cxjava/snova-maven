...
/**
 * This class provide utility methods for decompressing data blocks using the Snappy algorithm.
 * 
...
public class SnappyDecompressor {
	 ...
	 */
	public static SnappyBuffer decompress(byte[] in) {
	 ...
	 */
	public static SnappyBuffer decompress(byte[] in, SnappyBuffer out) {
	 ...
	 * @return decompressed data block
	 */
	public static SnappyBuffer decompress(byte[] in, int offset, int length) {
	 ...
	 * @return decompressed data block
	 */
	public static SnappyBuffer decompress(SnappyBuffer in) {
	 ...
	 * @return reference to <code>out</code>
	 */
	public static SnappyBuffer decompress(SnappyBuffer in, SnappyBuffer out) {
		return decompress(in.getData(), 0, in.getLength(), out);
	}

	/**
	 * Decompress the data contained in <code>in</code> from <code>offset</code> and <code>length</code> bytes. If an
	 * output buffer is provided, the buffer is reused for the decompressed data. If the buffer is too small, its
	 * capacity is expanded to fit the result. If a <code>null</code> argument is passed, a new buffer is allocated.
	 * 
	 * @param in
	 * @param offset
	 * @param length
	 * @param out
	 * @throws FormatViolationException
	 *             if the input data is invalid
	 * @return
	 */
	public static SnappyBuffer decompress(byte[] in, int offset, int length, SnappyBuffer out)
			throws FormatViolationException {
		...
		if (out == null) {
			out = new SnappyBuffer(targetLength);
		} else {
		...
		while (sourceIndex < offset + length) {
			...
			if (targetIndex >= targetLength) {
				throw new FormatViolationException("Superfluous input data encountered on offset " + sourceIndex,
						sourceIndex);
			}
