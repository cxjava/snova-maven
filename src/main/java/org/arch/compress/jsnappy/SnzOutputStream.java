...
public class SnzOutputStream extends OutputStream {
	 ...
	 *            target output stream
	 * @param bufferSize
	 *            buffer size must be a power of 2 between 2**0 and 2**29
	 * @throws IOException
	 */
	public SnzOutputStream(OutputStream out, int bufferSize) throws IOException {
		...
		cbuffer = new SnappyBuffer(this.bufferSize * 6 / 5);
	...
	/**
	 * Writes <code>length</code> bytes of data to the compressed stream from <code>data</code>, starting at index
	 * <code>offset</code>.
	 */
	@Override
	...
	/**
	 * Returns the compression effort used by this stream. The effort defaults to 1 (fastest, less compression).
	 * 
	 * @return
	 */
	...
	/**
	 * Sets the compression effort used by this stream from 1 (fastest, less compression) to 100 (slowest, best
	 * compression). If the effort is changed after the stream has been written to, the new effort will take effect on
	 * the next packet processed internally and may so affect data already written to the stream.
	 * 
	 * @param effort
	 */
	...
	/**
	 * Flushes the remaining data into a new compressed block, writes an end-of-stream marker and closes the underlaying
	 * output stream.
	 */
	@Override
