...
class TableBasedCompressor {
	...
	static SnappyBuffer compress(byte[] in, int offset, int length, SnappyBuffer out) {

		if (out == null) {
			out = new SnappyBuffer(length * 6 / 5);
		} else {
	...
	private static Hit search(byte[] source, int index, int length, int[] ilhm) {
		...
		if (index > 0 && source[index] == source[index - 1] && source[index] == source[index + 1]
				&& source[index] == source[index + 2] && source[index] == source[index + 3]) {
	...
	private static int toInt(byte[] data, int offset) {
		return (((data[offset] & 0xff) << 24) | ((data[offset + 1] & 0xff) << 16) | ((data[offset + 2] & 0xff) << 8) | (data[offset + 3] & 0xff)) & 0x7fffffff;
	}
