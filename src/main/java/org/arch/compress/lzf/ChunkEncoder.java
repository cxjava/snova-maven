...
/**
 * Class that handles actual encoding of individual chunks. Resulting chunks can be compressed or non-compressed;
 * compression is only used if it actually reduces chunk size (including overhead of additional header bytes)
 * 
 * @author tatu@ning.com
 */
public class ChunkEncoder {
	// Beyond certain point we won't be able to compress; let's use 16 bytes as
	// cut-off
	...
	public LZFChunk encodeChunk(byte[] data, int offset, int len) {
		if (len >= MIN_BLOCK_TO_COMPRESS) {
			/*
			 * If we have non-trivial block, and can compress it by at least 2 bytes (since header is 2 bytes longer),
			 * let's compress:
			 */
	...
	private int tryCompress(byte[] in, int inPos, int inEnd, byte[] out, int outPos) {
		...
		while (inPos < inEnd) {
			...
			if (ref < inPos && ref >= firstPos && (off = inPos - ref - 1) < MAX_OFF && in[ref + 2] == p2
					&& in[ref + 1] == (byte) (hash >> 8) && in[ref] == (byte) (hash >> 16)) {
				// match
