...
/**
 * Decoder that handles decoding of sequence of encoded LZF chunks, combining them into a single contiguous result byte
 * array
 * 
...
public class LZFDecoder {
	...
	public static byte[] decode(final byte[] sourceBuffer, int offset, int len) throws IOException {
		byte[] result = new byte[calculateUncompressedSize(sourceBuffer, offset, len)];
		decode(sourceBuffer, result);
		return result;
	}

	/**
	 * Method for decompressing whole input data, which encoded in LZF block structure (compatible with lzf command line
	 * utility), and can consist of any number of blocks
	 */
	public static int decode(final byte[] sourceBuffer, final byte[] targetBuffer) throws IOException {
		/*
		 * First: let's calculate actual size, so we can allocate exact result size. Also useful for basic sanity
		 * checking; so that after call we know header structure is not corrupt (to the degree that lengths etc seem
		 * valid)
		 */
		...
		while (inPtr < (sourceBuffer.length - 1)) { // -1 to offset possible end
													// marker
	...
	public static int decode(final byte[] sourceBuffer, int off, int srclen, final byte[] targetBuffer)
			throws IOException {
		/*
		 * First: let's calculate actual size, so we can allocate exact result size. Also useful for basic sanity
		 * checking; so that after call we know header structure is not corrupt (to the degree that lengths etc seem
		...
		while (inPtr < (srclen - 1)) { // -1 to offset possible end
										// marker
			inPtr += 2; // skip 'ZV' marker
			int type = sourceBuffer[off + inPtr];
			inPtr++;
			int len = uint16(sourceBuffer, off + inPtr);
			inPtr += 2;
			if (type == LZFChunk.BLOCK_TYPE_NON_COMPRESSED) { // uncompressed
				System.arraycopy(sourceBuffer, off + inPtr, result, outPtr, len);
				outPtr += len;
			} else { // compressed
				int uncompLen = uint16(sourceBuffer, off + inPtr);
				inPtr += 2;
				decompressChunk(sourceBuffer, off + inPtr, result, outPtr, outPtr + uncompLen);
				outPtr += uncompLen;
			}
			inPtr += len;
		}
	...
	private static int calculateUncompressedSize(byte[] data, int off, int len) throws IOException {
	 ...
	 * 
	 * @param is
	 *            An input stream of LZF compressed bytes
	 * @param inputBuffer
	 *            A byte array used as a scratch area.
	 * @param outputBuffer
