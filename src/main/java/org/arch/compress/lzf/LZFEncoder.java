...
/**
 * Encoder that handles splitting of input into chunks to encode, calls {@link ChunkEncoder} to compress individual
 * chunks and combines resulting chunks into contiguous output byte array.
 * 
...
public class LZFEncoder {
	...
	/**
	 * Method for compressing given input data using LZF encoding and block structure (compatible with lzf command line
	 * utility). Result consists of a sequence of chunks.
	 */
	...
	public static byte[] encode(byte[] data, int off, int length) throws IOException {
		...
		ChunkEncoder enc = new ChunkEncoder(left);
		int chunkLen = Math.min(LZFChunk.MAX_CHUNK_LEN, left);
		LZFChunk first = enc.encodeChunk(data, off, chunkLen);
		left -= chunkLen;
		// shortcut: if it all fit in, no need to coalesce:
		if (left < 1) {
			return first.getData();
		...
		do {
			chunkLen = Math.min(left, LZFChunk.MAX_CHUNK_LEN);
			LZFChunk chunk = enc.encodeChunk(data, off + inputOffset, chunkLen);
			inputOffset += chunkLen;
			left -= chunkLen;
			resultBytes += chunk.length();
			last.setNext(chunk);
