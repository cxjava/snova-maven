			...
			if (level == 1) {
				...
				if (cache == 0
					...
					if (source[off + o + 3] != source[off + src + 3]) {
						...
						int old_src = src;
						int remaining = ((length - UNCOMPRESSED_END - src + 1 - 1) > 255 ? 255 : (length
						...
						if (source[off + o + src - old_src] == source[off + src]) {
							...
							if (source[off + o + src - old_src] == source[off + src]) {
								src++;
								while (source[off + o + (src - old_src)] == source[off + src]
					...
					src++;
					dst++;
					fetch = ((fetch >>> 8) & 0xffff) | ((((int) source[off + src + 2]) & 0xff) << 16);
				}
			} else {
				fetch = (int) fast_read(source, off, src, 3);
				...
				byte c;
				int remaining = ((length - UNCOMPRESSED_END - src + 1 - 1) > 255 ? 255 : (length - UNCOMPRESSED_END
						- src + 1 - 1));
				int hash = ((fetch >>> 12) ^ fetch) & (HASH_VALUES - 1);
				...
				for (k = 0; k < QLZ_POINTERS_3 && (c > k || c < 0); k++) {
					o = hashtable[hash][k];
					if ((byte) fetch == source[off + o] && (byte) (fetch >>> 8) == source[off + o + 1]
							&& (byte) (fetch >>> 16) == source[off + o + 2] && o < src - MINOFFSET) {
						m = 3;
						while (source[off + o + m] == source[off + src + m] && m < remaining)
							m++;
				...
				if (dst <= last_matchstart) {
					...
					if (level == 1) {
						while (last_hashed < dst - 3) {
							last_hashed++;
							int fetch2 = (int) fast_read(destination, 0, last_hashed, 3);
