...
public class DnsClient {
	...
	private static final String[] rcodeDescription = { "No error", "DNS format error", "DNS server failure",
			"DNS name not found", "DNS operation not supported", "DNS service refused" };

	private static final int DEFAULT_PORT = 53;
	private InetAddress[] servers;
	private int[] serverPorts;
	private int timeout; // initial
							// timeout
							// on UDP
							// queries
							// in ms
	private int retries; // number of
							// UDP
							// retries
	...
	/*
	 * Each server is of the form "server[:port]". IPv6 literal host names include delimiting brackets. "timeout" is the
	 * initial timeout interval (in ms) for UDP queries, and "retries" gives the number of retries per server.
	 */
	...
	/*
	 * If recursion is true, recursion is requested on the query. If auth is true, only authoritative responses are
	 * accepted; other responses throw NameNotFoundException.
	 */
	ResourceRecords query(DnsName fqdn, int qclass, int qtype, boolean viaTcp, boolean recursion, boolean auth)
		...
		for (int retry = 0; retry < retries; retry++) {
			...
			for (int i = 0; i < servers.length; i++) {
				...
				try {
					...
					if ((null != hdr && hdr.truncated) || viaTcp) { // message is truncated -- try TCP
	...
	/**
	 * Tries to retreive an UDP packet matching the given xid received within the timeout. If a packet with different
	 * xid is received, the received packet is enqueued with the corresponding xid in 'resps'.
	 */
	private byte[] doUdpQuery(Packet pkt, InetAddress server, int port, int retry, int xid) throws IOException,
			NamingException {
		...
		synchronized (udpSocket) {
			...
			try {
				...
				do {
					if (debug) {
						cnt++;
						dprint("Trying RECEIVE(" + cnt + ") retry(" + (retry + 1) + ") for:" + xid
								+ "    sock-timeout:" + timeoutLeft + " ms.");
					}
	...
	/*
	 * Checks the header of an incoming DNS response. Returns true if it matches the given xid and throws a naming
	...
	private boolean isMatchResponse(byte[] pkt, int xid) throws NamingException {
		...
		if (debug) {
			dprint("NO-MATCH SEND ID:" + xid + " RECVD ID:" + hdr.xid + "    Response Q:" + resps + "    Reqs size:"
					+ reqs.size());
		}
		return false;
	}
	...
	/*
	 * Throws an exception if appropriate for the response code of a given header.
	 */
