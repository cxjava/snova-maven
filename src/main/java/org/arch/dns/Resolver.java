...
public class Resolver {
	...
	public static String[] resolveIPv4(String[] dnsServer, String host, ResolveOptions option) throws NamingException {
		if (null == option) {
		...
		if (option.cacheTtl != 0) {
			...
			if (null != item) {
				if (option.cacheTtl == ResolveOptions.DNS_CACHE_TTL_FOREVER
						|| (item.retirveTime + item.ttl >= System.currentTimeMillis())) {
					return item.ips;
				}
			}

		}
		DnsClient client = new DnsClient(dnsServer, option.retry, option.timeout);
		ResourceRecords records = client.query(new DnsName(host), ResourceRecord.TYPE_A, ResourceRecord.CLASS_INTERNET,
