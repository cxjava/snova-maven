...
/**
 * The ResourceRecord class represents a DNS resource record. The string format is based on the master file
 * representation in RFC 1035.
 * 
...
public class ResourceRecord {
	 ...
	 */
	static final String rrTypeNames[] = { null, "A", "NS", null, null, "CNAME", "SOA", null, null, null, null, null,
			"PTR", "HINFO", null, "MX", "TXT", null, null, null, null, null, null, null, null, null, null, null,
			"AAAA", null, null, null, null, "SRV", null, "NAPTR" };
	...
	/*
	 * Constructs a new ResourceRecord. The encoded data of the DNS message is contained in msg; data for this RR begins
	 * at msg[offset]. If qSection is true this RR is part of a question section. It's not a true resource record in
	 * that case, but is treated as if it were a shortened one (with no ttl or rdata). If decodeRdata is false, the
	 * rdata is not decoded (and getRdata() will return null) unless this is an SOA record.
	 ...
	 */
	ResourceRecord(byte[] msg, int msgLen, int offset, boolean qSection, boolean decodeRdata)
	...
	/*
	 * Compares two SOA record serial numbers using 32-bit serial number arithmetic as defined in RFC 1982. Serial
	 * numbers are unsigned 32-bit quantities. Returns a negative, zero, or positive value as the first serial number is
	 * less than, equal to, or greater than the second. If the serial numbers are not comparable the result is
	 * undefined. Note that the relation is not transitive.
	 */
	public static int compareSerialNumbers(long s1, long s2) {
		long diff = s2 - s1;
		if (diff == 0) {
			return 0;
		} else if ((diff > 0 && diff <= 0x7FFFFFFF) || (diff < 0 && -diff > 0x7FFFFFFF)) {
			return -1;
		} else {
			return 1;
		}
	...
	/*
	 * Decodes the binary format of the RR. May throw ArrayIndexOutOfBoundsException given corrupt data.
	 */
	private void decode(boolean decodeRdata) throws InvalidNameException {
		...
		if (!qSection) {
			...
			(rrtype == TYPE_SOA)) ? decodeRdata(pos) : null;
	...
	/*
	 * Returns the 2-byte unsigned value at msg[pos]. The high order byte comes first.
	 */
	...
	/*
	 * Prepends to "n" the domain name encoded at msg[pos], including the root label. Returns the index into "msg"
	 * following the name.
	 */
	...
	/*
	 * Returns the rdata encoded at msg[pos]. The format is dependent on the rrtype and rrclass values, which have
	 * already been set. The length of the encoded data is rdlen, which has already been set. The rdata of records with
	 * unknown type/class combinations is returned in a newly-allocated byte array.
	 */
	...
	/*
	 * Returns the rdata of an SRV record that is encoded at msg[pos]. See RFC 2782.
	 */
	...
	/*
	 * Returns the rdata of an NAPTR record that is encoded at msg[pos]. See RFC 2915.
	 */
	...
	/*
	 * Returns the rdata of a TXT record that is encoded at msg[pos]. The rdata consists of one or more
	 * <character-string>s.
	 */
	...
	/*
	 * Returns the rdata of an HINFO record that is encoded at msg[pos]. The rdata consists of two <character-string>s.
	 */
	...
	/*
	 * Decodes the <character-string> at msg[pos] and adds it to buf. If the string contains one of the meta-characters
	 * ' ', '\\', or '"', then the result is quoted and any embedded '\\' or '"' chars are escaped with '\\'. Empty
	 * strings are also quoted. Returns the size of the encoded string, including the initial length octet.
	 */
	...
	/*
	 * Returns the rdata of an A record, in dotted-decimal format, that is encoded at msg[pos].
	 */
	private String decodeA(int pos) {
		return ((msg[pos] & 0xff) + "." + (msg[pos + 1] & 0xff) + "." + (msg[pos + 2] & 0xff) + "." + (msg[pos + 3] & 0xff));
	}

	/*
	 * Returns the rdata of an AAAA record, in colon-separated format, that is encoded at msg[pos]. For example:
	 * 4321:0:1:2:3:4:567:89ab. See RFCs 1886 and 2373.
	 */
