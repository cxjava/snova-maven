...
/**
 * The ResourceRecords class represents the resource records in the four sections of a DNS message.
 * 
...
class ResourceRecords {
	...
	/*
	 * True if these resource records are from a zone transfer. In that case only answer records are read (as per
	 * draft-ietf-dnsext-axfr-clarify-02.txt). Also, the rdata of those answer records is not decoded (for efficiency)
	 * except for SOA records.
	 */
	boolean zoneXfer;

	/*
	 * Returns a representation of the resource records in a DNS message. Does not modify or store a reference to the
	 * msg array.
	 */
	ResourceRecords(byte[] msg, int msgLen, Header hdr, boolean zoneXfer) throws NamingException {
		if (zoneXfer) {
	...
	/*
	 * Returns the type field of the first answer record, or -1 if there are no answer records.
	 */
	int getFirstAnsType() {
		if (answer.size() == 0) {
			return -1;
	...
	/*
	 * Returns the type field of the last answer record, or -1 if there are no answer records.
	 */
	int getLastAnsType() {
		if (answer.size() == 0) {
			return -1;
	...
	/*
	 * Decodes the resource records in a DNS message and adds them to this object. Does not modify or store a reference
	 * to the msg array.
	 */
	void add(byte[] msg, int msgLen, Header hdr) throws NamingException {
		...
		try {
			...
			for (int i = 0; i < hdr.numAnswers; i++) {
				rr = new ResourceRecord(msg, msgLen, pos, false, !zoneXfer);
