		...
		try {
			...
			case FASTLZ: {
				...
				try {
					...
					int decompressed = fastlz.fastlzDecompress(raw, buffer.getReadIndex(), buffer.readableBytes(),
							newbuf, 0, len);
			...
			case SNAPPY: {
				try {
					SnappyBuffer newbuf = SnappyDecompressor.decompress(raw, buffer.getReadIndex(),
