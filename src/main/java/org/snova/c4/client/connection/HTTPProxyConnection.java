...
public class HTTPProxyConnection extends ProxyConnection {
	...
	private ChannelFuture connectProxyServer() {
		...
		pipeline.addLast("executor", new ExecutionHandler(SharedObjectHelper.getGlobalThreadPool()));
	...
	private synchronized ChannelFuture getRemoteChannelFuture() {
		if (null == clientChannelFuture
				|| (clientChannelFuture.isSuccess() && !clientChannelFuture.getChannel().isConnected())) {
	...
	protected void sendContent(Channel ch, final Buffer content) {
		...
		request.setHeader("C4MiscInfo", role + "_" + cfg.getHTTPRequestTimeout() + "_" + cfg.getMaxReadBytes());
		request.setHeader("Content-Length", String.valueOf(content.readableBytes()));
		if (content.readableBytes() > 0) {
			ChannelBuffer buffer = ChannelBuffers.wrappedBuffer(content.getRawBuffer(), content.getReadIndex(),
	...
	class HttpResponseHandler extends SimpleChannelUpstreamHandler {
		...
		private boolean tryHandleBuffer() {
			if (responseContentLength == -1) {
				if (resBuffer.readableBytes() >= 4) {
					responseContentLength = BufferHelper.readFixInt32(resBuffer, true);
					if (responseContentLength <= 0 || responseContentLength > 2 * cfg.getMaxReadBytes()) {
						logger.error("##############Invalid length :" + responseContentLength + ":"
								+ resBuffer.readableBytes() + " for session:" + getSession().getSessionID());
						return false;
					}
				} else {
					return false;
		...
		public void messageReceived(ChannelHandlerContext ctx, MessageEvent e) throws Exception {
			...
			if (msg instanceof HttpResponse) {
				...
				String theader = response.getHeader(HttpHeaders.Names.TRANSFER_ENCODING);
				hasTranferEncoding = theader != null && theader.equalsIgnoreCase("chunked");
				...
				if (response.getStatus().getCode() == 200) {
					...
					logger.error("Received error response:" + response + " for isPull:" + isPullConnection);
				...
				logger.error("Unsupported message type:" + msg.getClass().getCanonicalName());
			}
		}
	}
}
