...
public class ProxyConnectionManager {
	...
	public boolean init(List<C4ServerAuth> gaeAuths) {
		...
		for (C4ServerAuth auth : C4ClientConfiguration.getInstance().getC4ServerAuths()) {
	...
	public boolean recycleProxyConnection(ProxyConnection connection) {
		...
		LinkedBlockingDeque<ProxyConnection> list = freeConnTable.get(connection.getC4ServerAuth().domain);
		if (list.size() >= 30) {
			connection.close();
			return false;
		}
	...
	public ProxyConnection getClientConnectionByAuth(C4ServerAuth auth) {
		...
		LinkedBlockingDeque<ProxyConnection> connlist = freeConnTable.get(auth.domain);
		if (null == connlist) {
			connlist = new LinkedBlockingDeque<ProxyConnection>();
			freeConnTable.put(auth.domain, connlist);
		}
		ConnectionMode mode = C4ClientConfiguration.getInstance().getConnectionMode();
		if (!connlist.isEmpty()) {
	...
	public ProxyConnection getClientConnectionByAuth(C4ServerAuth auth, HTTPRequestEvent event) {
		return getClientConnectionByAuth(auth);
	}

	public ProxyConnection[] getDualClientConnection(HTTPRequestEvent event) {
		String domain = null != event ? C4ClientConfiguration.getInstance().getBindingDomain(event.getHeader("Host"))
				: null;
