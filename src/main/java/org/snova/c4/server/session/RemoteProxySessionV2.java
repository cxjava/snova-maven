...
public class RemoteProxySessionV2 {
	protected static Logger logger = LoggerFactory.getLogger(RemoteProxySessionV2.class);
	...
	public void extractEventResponses(Buffer buf, int maxSize, LinkedList<Event> evs) {
	...
	public static RemoteProxySessionV2 dispatchEvent(String user, final Buffer content) throws Exception {
	...
	private boolean handleEvent(TypeVersion tv, Event ev) {
		...
		default: {
			logger.error("Unsupported event type version " + tv.type + ":" + tv.version);
			return false;
		}
		}
		return true;
