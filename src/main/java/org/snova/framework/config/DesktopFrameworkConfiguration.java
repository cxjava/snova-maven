 ...
 * 
 */
public class DesktopFrameworkConfiguration implements FrameworkConfiguration, ReloadableConfiguration {
	...
	private SimpleSocketAddress localServerAddress = new SimpleSocketAddress("localhost", 48100);
	...
	private String[] trsutedDNS = new String[] { "8.8.8.8", "208.67.222.222", "8.8.4.4", "208.67.220.220" };
	...
	private static File getConfigFile() {
		URL url = DesktopFrameworkConfiguration.class.getResource("/" + CONF_FILE);
		String conf;
		try {
	...
	private void loadConfig() {
		InputStream is = DesktopFrameworkConfiguration.class.getResourceAsStream("/" + CONF_FILE);
		if (null != is) {
			IniProperties props = new IniProperties();
			try {
				props.load(is);
				proxyService = props.getProperty(TAG, PROXY_SERVICE_NAME, proxyService);
				threadPoolSize = props.getIntProperty(TAG, THREAD_POOL_SIZE_NAME, threadPoolSize);
				localServerAddress.host = props.getProperty(TAG, PROXY_SERVER_HOST, localServerAddress.host);
				localServerAddress.port = props.getIntProperty(TAG, PROXY_SERVER_PORT, localServerAddress.port);
				...
				pluginRepository = props.getProperty(TAG, PLUGIN_REPOSITORY, pluginRepository);
			} catch (Exception e) {
				logger.error("Failed to load config file:" + CONF_FILE, e);
			}
	...
	public void save() {
		...
		try {
			...
			props.setIntProperty(TAG, PROXY_SERVER_PORT, localServerAddress.port);
